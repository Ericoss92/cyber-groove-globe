...
function DiscoverPage() {
  const { data, loading, error, hasCatalog } = useDiscoverData();
  ...
  return (
    <div className="mx-auto max-w-[1600px] px-4 md:px-6 py-8 space-y-12">
      ...
      {/* SECTION 1 — recent */}
      <section>
        <SectionTitle kicker="// RECENT" title="Tes 10 derniers sons" />
        {loading ? (
      ...
      <section>
        <SectionTitle kicker="// WEEK TOP" title="Top artistes cette semaine" />
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground font-mono">// pas assez de données cette semaine</p>
        )}
      </section>
